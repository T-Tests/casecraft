...
def test_chunk_text_basic():
    text = "x" * 2000
    chunks = parser._chunk_text(text, chunk_size=800, overlap=100)
...
def test_chunk_text_invalid_overlap():
    with pytest.raises(ValueError):
        parser._chunk_text("text", chunk_size=100, overlap=100)

    with pytest.raises(ValueError):
        parser._chunk_text("text", chunk_size=0, overlap=0)

    with pytest.raises(ValueError):
        parser._chunk_text("text", chunk_size=100, overlap=-1)
...
def test_chunk_handles_long_token():
    ...
    long_word = "A" * 1000
    chunks = parser._chunk_text(long_word, chunk_size=100, overlap=10)
